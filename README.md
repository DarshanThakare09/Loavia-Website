...
Clear
Contents
🍪 LOAVIA — Project Requirements Document*Healthy Inside, Yummy Outside*📋 Table of Contents1. Project Overview1.1 Mission1.2 Target Experience1.3 Business Objectives1.4 Design Philosophy2. Technology StackFrontendBackendServicesDeploymentArchitecture Type3. Complete Page Architecture3.1 Public Pages📄 Page 1 — Homepage `/`📄 Page 2 — Shop / Product Listing `/shop`📄 Page 3 — Product Detail Page `/product/:slug`📄 Page 4 — Login / Register `/auth`📄 Page 5 — About `/about`📄 Page 6 — Contact `/contact`3.2 Authenticated User Pages📄 Page 7 — Cart `/cart`📄 Page 8 — Checkout `/checkout`📄 Page 9 — Order Confirmation & Tracking `/orders/:id`📄 Page 10 — Order History `/orders`📄 Page 11 — Wishlist `/wishlist`📄 Page 12 — User Profile `/profile`📄 Page 13 — Build Your Box `/build-box`📄 Page 14 — Gift Page `/gift`3.3 Admin Panel `/admin`🔧 Admin 1 — Dashboard `/admin/dashboard`🔧 Admin 2 — Product Management `/admin/products`🔧 Admin 3 — Order Management `/admin/orders`🔧 Admin 4 — User Management `/admin/users`🔧 Admin 5 — Offers & Coupons `/admin/offers`4. Core System Logic & Backend Requirements4.1 Authentication System4.2 Product & Pricing Logic4.3 Cart System4.4 Checkout — Exact Transactional Sequence4.5 Order Management System (OMS)4.6 Review & Rating System4.7 Loyalty & Rewards System4.8 Recommendation Engine4.9 Cookie Customization — Build Your Box Logic4.10 Edge Cases & Race Condition Handling5. Database SchemaCollections`users``products``cart``orders``order_items``reviews``coupons``wishlist``loyalty_ledger``categories`6. API EndpointsAuth RoutesProduct RoutesCart RoutesCheckout & Order RoutesWishlist RoutesReview RoutesUser RoutesCoupon & Offers RoutesAdmin Routes7. UI/UX & Animation Requirements7.1 Navigation Bar7.2 Hero Section7.3 Product Cards7.4 Scroll Animations (GSAP ScrollTrigger)7.5 Page Load & Transitions7.6 Color Palette7.7 Typography7.8 Responsive Breakpoints8. Payment IntegrationGatewayFlowInvoice Generation9. NotificationsEmail Notifications (Required — Phase 1)SMS Notifications (Phase 2 / Optional)In-App NotificationsAdmin Automated Alerts10. Advanced & Future-Ready FeaturesSubscription Cookie BoxesReal-Time Stock UpdatesSEO OptimizationFuture Mobile App APIAdvanced Recommendation EngineOther11. Development Phases & Delivery OrderCommon Mistakes to Avoid12. Pre-Development Handoff ChecklistProject Summary
...
Core System Logic & Backend Requirements
Database Schema
API Endpoints
...
Pre-Development Handoff Checklist
1. Project Overview#
...
Drive impulse purchases through visual, emotion-first design
Enable gifting use-cases with occasion-based customization
Provide an interactive brand experience through animations and 3D elements
Build repeat customers through a loyalty and reward points system
Maintain a scalable, API-first backend architecture ready for future mobile app integration
...
Fixed Navigation Bar — logo, nav links, search, account icon, cart icon with badge
Hero Section — full-viewport, 3D cookie element (Three.js), bold headline, "Shop Now" CTA, GSAP entrance animation
Shop by Mood — interactive mood-based browsing (e.g., Craving Something Sweet, Gifting, Healthy)
Best Sellers — product card grid with hover interactions
Categories — visual category tiles
Build Your Box Highlight — CTA banner linking to the custom box builder
Gifting Section — occasion-based gifting CTA
Limited Edition / Seasonal — time-sensitive product highlight
Customer Testimonials — star ratings + review previews
Footer — logo, quick links, support links, social icons, copyright
...
Address — Select saved address or add new. Validate ZIP/phone via regex.
Delivery Method — Available shipping options with estimated delivery dates
Apply Offers — Coupon codes, auto-applicable discounts
Payment — Razorpay/Stripe integration (UPI, Cards, Net Banking)
Order Summary — Final review with all details before placing order
...
Before writing the first line of code, confirm ALL of the following:
 ...
 Razorpay account **Razorpay account** set up (or in progress) by client
 Cloudinary / AWS S3 **Cloudinary / AWS S3** account for image storage confirmed
